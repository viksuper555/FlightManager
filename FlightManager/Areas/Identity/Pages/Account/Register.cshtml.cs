...
namespace FlightManager.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = new Manager { UserName = Input.Username, Email = Input.Email, 
                    FirstName = Input.FirstName, LastName = Input.LastName, 
                    EGN = Input.EGN, Address = Input.Address, PhoneNumber = Input.PhoneNumber };
                var result = await _userManager.CreateAsync(user, Input.Password);
                if (result.Succeeded)
                {
                    if (_userManager.Users.Count() <= 1) //If no users registered
                    {
